...
class Components:
    ...
    async def publish_warlog_embed(self, ctx: CommandContext | ComponentContext, clan_tag: str, page: int, edit: bool) -> None:
        ...
        max_pages = [(len(warlog_response) // self.variables.wars_per_page) + 1 if len(warlog_response) % self.variables.wars_per_page else len(
            warlog_response) // self.variables.wars_per_page][0]
        page = [1 if page <= 0 else max_pages if page > max_pages else page][0]
        start, end = (page - 1) * self.variables.wars_per_page, page * self.variables.wars_per_page
        clan_embed = Embed(
    ...
    async def warlog_previous_page(self, ctx: ComponentContext) -> None:
        clan_tag = ctx.message.embeds[0].title.split(' ')[-1].strip('#')
        page = int(ctx.message.embeds[0].footer.text.split(" ")[1]) - 1
        await self.publish_warlog_embed(ctx, clan_tag, page, True)

    async def warlog_next_page(self, ctx: ComponentContext) -> None:
        clan_tag = ctx.message.embeds[0].title.split(' ')[-1].strip('#')
        page = int(ctx.message.embeds[0].footer.text.split(" ")[1]) + 1
        await self.publish_warlog_embed(ctx, clan_tag, page, True)
