...
class AutoCompletes:
    def __init__(self):
        self.clan = Clan()
        self.player = Player()
        self.current_war = ClanWar()

    async def player_tag_auto_complete(self, ctx: CommandContext, user: User, input_player_tag: str = None) -> None:
        """
        ...
        if clan_tag is not None:
            choices = [
                Choice(
                    ...
                    value=member['tag']
                ) for member in (await self.clan.members(clan_tag))['items']
        ...
        if input_player_tag is not None:
            ...
            player_response = await self.player.player(input_player_tag)
        ...
        await ctx.populate([elem for i, elem in enumerate(choices) if
                            (elem.name, elem.value) not in [(ch.name, ch.value) for ch in choices[:i]]][:25])

    @staticmethod
    async def player_auto_complete(ctx: CommandContext, user: User, input_player: str = None) -> None:
        """

        :param ctx: CommandContext
        :param user: User
        :param input_player: str = None        :return:
        """
    ...
    async def clan_tag_auto_complete(self, ctx: CommandContext, user: User, input_clan_tag: str = None) -> None:
        clan_tag = user.guilds.fetch_clantag(ctx.guild_id)
        choices = []
        if clan_tag is not None:
            ...
            if 'reason' not in clan_response:
                ...
                war_response = await self.current_war.current_war(clan_tag)
                if war_response['state'] != 'notInWar':
                    opponent = await self.clan.clan(war_response['opponent']['tag'])
            ...
            if input_clan_tag is not None:
                ...
                input_response = await self.clan.clan(input_copy)
                    ...
                    search_choices = [
                        Choice(
                            name=f"{item['name']} {item['tag']}, "
                                 ...
                                 f"{item['members']}/50",
                            value=str(item['tag'])
                        ) for item in (await self.clan.clan_search(input_clan_tag, limit=25))['items']
                    ]
                    choices += search_choices
        await ctx.populate([elem for i, elem in enumerate(choices) if
                            (elem.name, elem.value) not in [(ch.name, ch.value) for ch in choices[:i]]][:25])

    async def clan_auto_complete(self, ctx: CommandContext, user: User, input_clan: str = None) -> None:
        clan_tags = user.guilds.fetch_clantags(ctx.guild_id)
        if len(clan_tags) == 0:
            await ctx.populate([])
        choices = [Choice(name=c_response) for c_response in await self.clan.clan_bulk(clan_tags)]

        if input_clan is None:
            choices = [elem for i, elem in enumerate(choices) if
                       (elem.name, elem.value) not in [(ch.name, ch.value) for ch in choices[:i]]][:25]
        else:
            choices = [elem for i, elem in enumerate(choices) if
                       (elem.name, elem.value) not in [(ch.name, ch.value) for ch in choices[:i]] and input_clan in elem.name or input_clan in elem.value][:25]
