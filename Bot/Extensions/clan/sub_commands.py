...
class SubCommands:
    ...
    async def table(self, ctx: CommandContext, clan_tag: str, sort: int = 0, descending: bool = False) -> None:
        ...
        members_table_list = [
            [
                [position['clanRank'] for position in response_clan['memberList'] if position['tag'] == member['tag']][
                    0],
                member['name'],
                member['trophies'],
                ...
                member['expLevel'],
                member['tag']
            ] for member in await self.player.player_bulk([member["tag"] for member in response_clan["memberList"]])
        ]
        ...
        await first_message.edit("".join((
            "`#    name", (max_name_len - 2) * ' ',
            "trophies  war  stars  donations  received  th  role       level  tag",
            (max_tag_len - 3) * ' ', "`\n",
